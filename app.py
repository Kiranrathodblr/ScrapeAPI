...
def index():
    if request.method == 'POST':
        ...
        if free_status != True:
            ...
            free_status = True
            searchString = request.form['InputUrl'].replace(" ", "")  # obtaining the search string entered in the form
          #  expected_view_video = request.form['Scrapstring']
        try:
            #expected_view_video = 0
            scrapper_object = FlipkratScrapper(executable_path=ChromeDriverManager().install(), chrome_options=chrome_options)
           # mongoClient = MongoDBManagement(username='', password='')
           # scrapper_object.openUrl("https://www.flipkart.com/")
            scrapper_object.openUrl('searchString')
...
def index():
    '''               '''if request.method == 'POST':
        ...
        try:
            ...
            #scrapper_object.openUrl("https://www.flipkart.com/")
            scrapper_object.openUrl("https://www.youtube.com/user/krishnaik06/")
            logger.info("Url hitted")
            #scrapper_object.login_popup_handle()
            logger.info("login popup handled")
           # scrapper_object.searchProduct(searchString=searchString)
...
if __name__ == "__main__":
    #app.run(port=5000,host='127.0.0.1')  # running the app on the local machine on port 8000
    app.run()